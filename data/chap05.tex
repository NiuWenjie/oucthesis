...
% 多样性我们选择Learned Perceptual Image Patch Similarity (LPIPS)来评价。预训练好的AlexNet~\cite{krizhevsky2017imagenet}网络来提取特征后计算$L_1$距离，同一个输入的多个结果对之间计算平均距离，对于测试图像求平均值作为LPIPS结果。在进行多种样式域翻译实验中，输入会翻译成多个样式域的结果
